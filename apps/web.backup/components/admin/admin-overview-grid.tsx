...
import { createSwapy, utils, type SlotItemMapArray, type SwapEvent, type Swapy } from 'swapy';
...
export function AdminOverviewGrid({ pages }: AdminOverviewGridProps) {
  ...
  useEffect(() => {
    ...
    swapy.onSwap((event: SwapEvent) => {
      setDraftLayout(event.newSlotItemMap.asArray as AdminOverviewLayoutEntry[]);
    });
  ...
  const slottedPages = utils.toSlottedItems<AdminOverviewWorkspace>(pages, 'key', layout as SlotItemMapArray);
