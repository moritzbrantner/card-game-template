    ...
    <div className="space-y-6">
            ...
            {interactionKeys.map((key) => (
              ...
              >
                <h3 className="font-semibold text-zinc-950 dark:text-zinc-50">
                  {t(`interactions.${key}.title`)}
                </h3>
                <div className="mt-3 space-y-2 text-sm leading-6 text-zinc-600 dark:text-zinc-300">
                  <p>
                    <strong>Required:</strong>{' '}
                    {t(`interactions.${key}.required`)}
                  </p>
                  <p>
                    <strong>Dirty:</strong> {t(`interactions.${key}.dirty`)}
                  </p>
                  <p>
                    <strong>Validity:</strong>{' '}
                    {t(`interactions.${key}.validity`)}
                  </p>
                  <p>
                    <strong>Reset:</strong> {t(`interactions.${key}.reset`)}
                  </p>
                </div>
              </article>
            ))}
      ...
      <Card className="mx-auto max-w-4xl rounded-3xl">
        <CardHeader>
          <CardTitle>{t('title')}</CardTitle>
          <CardDescription>
            This page demonstrates react-hook-form with ten fields and built-in
            validation.
          </CardDescription>
        </CardHeader>
        <CardContent>
