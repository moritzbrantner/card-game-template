...
test.describe('admin reports', () => {
  test('shows operational pulse cards on the admin overview and links to report details', async ({
    page,
  }) => {
    ...
    await expect(page).toHaveURL(
      /\/en\/admin\/reports\/securityAccess\?window=7d/,
    );
  });

  test('opens a live report detail page and exports CSV data', async ({
    page,
  }) => {
    await loginWithCredentials(page, adminUser.email, adminUser.password);
    ...
    await expect(
      page.getByRole('heading', { name: 'Security access review' }),
    ).toBeVisible();
    await expect(
