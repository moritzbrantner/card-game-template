...
test('the employee REST table page displays, sorts, and filters data', async ({
  page,
}) => {
  await gotoAndWaitForHydration(page, '/en/table');

  await expect(
    page.getByRole('heading', { name: 'Employee table' }),
  ).toBeVisible();
  await expect(
    page.getByRole('columnheader', { name: 'First name' }),
  ).toBeVisible();
  await expect(
    page.getByRole('cell', { name: 'Ava', exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole('cell', { name: 'ava.thompson@example.com' }),
  ).toBeVisible();
