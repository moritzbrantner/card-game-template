...
test('renders the default desktop route with a shared package label', async ({
  page,
}) => {
  await expect(
    page.getByRole('heading', { name: 'Hello Electron!' }),
  ).toBeVisible();
...
test('persists the selected theme across relaunch', async ({ app, page }) => {
  ...
  await expect
    .poll(() =>
      relaunchedPage.evaluate(() => document.documentElement.dataset.theme),
    )
    .toBe('dark');
...
test('saves and reopens a document through the command-backed document flow', async ({
  page,
}) => {
  ...
  await expect(
    page.getByText(/Saved e2e-document\.desktop\.json/),
  ).toBeVisible();
  ...
  await page
    .getByRole('button', { name: /e2e-document\.desktop\.json/ })
    .click();
