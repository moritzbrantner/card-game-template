...
describe('@moritzbrantner/storytelling', () => {
  ...
  test('renders a story container in jsdom', () => {
    ...
    expect(
      screen.getByRole('navigation', { name: 'Story minimap' }),
    ).toBeTruthy();
  ...
  test('navigates story scenes through the minimap', () => {
    ...
    try {
      ...
      fireEvent.click(
        screen.getAllByRole('button', { name: 'Go to scene 2: Two' })[0],
      );
  ...
  test('renders a branching story and advances when a choice is selected', async () => {
    ...
    expect(screen.getAllByText('Wake the observatory').length).toBeGreaterThan(
      0,
    );
...
The archive starts speaking again.`,
    ...
    try {
      ...
      expect(
        await screen.findByText('We open on the empty station.'),
      ).toBeTruthy();
      expect(
        screen.getByText('The archive starts speaking again.'),
      ).toBeTruthy();
  ...
  test('renders audio and video story scenes with native media elements', async () => {
    ...
    expect(
      audio?.querySelector('track[kind="captions"]')?.getAttribute('src'),
    ).toBe('/audio/transmission.vtt');
    ...
    expect(
      video
        ?.querySelector('track[label="English subtitles"]')
        ?.getAttribute('src'),
    ).toBe('/video/feed.vtt');
  });
});
