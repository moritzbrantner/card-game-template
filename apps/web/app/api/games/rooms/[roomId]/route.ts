import { getGameRoomUseCase } from '@/src/domain/game-rooms/use-cases';
...
export const GET = createApiRoute({
  action: 'games.rooms.get',
  featureKey: 'showcase.uno',
  async handler({ request, session }) {
    const result = await getGameRoomUseCase(session, getRoomId(request));

    if (!result.ok) {
      throw mapRoomProblem('Unable to load room', result.error.code, result.error.message);
    }
