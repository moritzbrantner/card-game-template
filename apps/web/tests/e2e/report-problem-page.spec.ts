...
test('the user can open the report problem page and submit a report', async ({
  page,
}) => {
  ...
  await page
    ...
    .fill('Dashboard freezes after filters change');
  await page
    .getByLabel('What happened?')
    .fill(
      'Every time I update the region filter, the dashboard hangs for about thirty seconds before recovering.',
    );
