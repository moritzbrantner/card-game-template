...
test('subtree sync workflow validates payload mappings and syncs only one prefix', () => {
  assert.match(
    workflowSource,
    /Prefix \$\{prefix\} does not match source_repo/,
  );
  ...
  assert.match(
    workflowSource,
    /bash scripts\/sync-subtrees\.sh --apps --prefix "\$\{PREFIX\}"/,
  );
...
test('subtree sync workflow skips PR creation when the head would match develop or no commits exist', () => {
  ...
  assert.match(
    workflowSource,
    /git rev-list --count "origin\/develop\.\.origin\/\$\{SYNC_BRANCH\}"/,
  );
  assert.match(
    workflowSource,
    /Skipping subtree sync PR because there are no commits between develop and/,
  );
});
...
test('README and infrastructure docs record the subtree sync automation secrets', () => {
  ...
  assert.match(
    readmeSource,
    /opens or updates a pull request back into `develop`/,
  );
