...
| ----- | --------- | --------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| P-001 | completed | Shared game contracts       | Define generic moves, turns, players, rooms, and result models    | Contracts are documented, typed, and reused across apps and server code     | `game-contracts`, `auth-contract`, and `multiplayer-contract` exist                              |
...
- `packages/game-engine`: deterministic serial turn engine, inspection helpers, move validation/canonicalization, seeded RNG, and transition summaries
- `packages/card-kit`: card identities, card visibility helpers, and reusable stack/deck utilities
- `packages/game-session`: local sessions, server-authoritative sessions, replay reconstruction, and replay integrity checks
- `packages/game-uno`, `packages/game-poker`, `packages/game-tcg`, `packages/game-tic-tac-toe`: sample game adapters with local sessions, bots, and unit/fixture coverage
