...
| P-003 | in_progress | Account foundation          | Add account, session, and player profile model                    | A user can authenticate and resolve to one player identity across platforms | Web account/guest ownership exists; mobile/desktop adapters remain    |
| P-004 | in_progress | Online multiplayer backbone | Add rooms, move submission, synchronization, and server authority | Multiple players can complete a match online with validated moves           | Server-authoritative sessions and web UNO moves exist; rooms/realtime remain |
| P-005 | in_progress | Result tracking             | Persist match outcomes and surface player history                 | Completed matches create durable records tied to accounts and games         | Web UNO persistence, move logs, replay inspection, and history exist  |
